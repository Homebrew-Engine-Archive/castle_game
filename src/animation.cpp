...
AnimationBuilder::AnimationBuilder(const gm1::Collection &collection, const std::vector<Frame> &frames)
    : gm1(collection)
    , frames(frames)
    , position(0)
{
...
void AnimationBuilder::Read(
    size_t frameCount,
    const std::vector<Direction> &dirs,
    std::map<Direction, std::vector<Frame>> &result)
{
